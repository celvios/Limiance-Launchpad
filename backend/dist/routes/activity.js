...
async function activityRoutes(app) {
    app.get('/api/activity', {
        ...
        const profileWhere = walletAddresses.length > 0
            ? { OR: walletAddresses.map((walletAddress) => ({ walletAddress: { equals: walletAddress, mode: 'insensitive' } })) }
            : { walletAddress: { in: [] } };
        const [tokenRows, profiles] = await Promise.all([
            ...
            prisma_1.prisma.profile.findMany({ where: profileWhere, select: { walletAddress: true, username: true, usernameDisplay: true } }),
        ]);
        ...
        const usernameFor = (walletAddress) => profileMap.get(walletAddress.toLowerCase()) ?? null;
