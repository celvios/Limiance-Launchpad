...
exports.adminDataRoutes = adminDataRoutes;
...
function serializeBigInt(row) {
    return Object.fromEntries(Object.entries(row).map(([key, value]) => [key, typeof value === 'bigint' ? value.toString() : value]));
}
async function adminDataRoutes(app) {
    app.get('/api/admin/dashboard', async (request, reply) => {
        const admin = await (0, adminAuth_1.requireAdmin)(request, ['support_admin', 'token_admin', 'finance_admin', 'viewer']);
        if (!admin)
        ...
        const [users, profiles, tokens, trades24h, comments, openReports, pendingWithdrawals, indexer] = await Promise.all([
            ...
            prisma_1.prisma.profile.count({ where: { onboarded: true } }),
            prisma_1.prisma.token.count(),
            prisma_1.prisma.trade.count({ where: { timestamp: { gte: since } } }),
            prisma_1.prisma.comment.count(),
            prisma_1.prisma.report.count({ where: { status: { in: ['open', 'reviewing'] } } }),
            prisma_1.prisma.withdrawalRequest.count({ where: { status: 'pending' } }),
            prisma_1.prisma.indexerState.findUnique({ where: { id: 'deposit_indexer' } }),
        ]);
        const balanceAggregate = await prisma_1.prisma.userBalance.aggregate({ _sum: { available: true } });
        return reply.send({
    ...
    app.get('/api/admin/users', async (request, reply) => {
        const admin = await (0, adminAuth_1.requireAdmin)(request, ['support_admin', 'finance_admin', 'viewer']);
        if (!admin)
            return reply.code(403).send({ error: 'Forbidden' });
        ...
        const [profiles, balances] = await Promise.all([
            prisma_1.prisma.profile.findMany({ where: { walletAddress: { in: wallets } }, select: { walletAddress: true, usernameDisplay: true, username: true, onboarded: true } }),
            prisma_1.prisma.userBalance.findMany({ where: { walletAddress: { in: wallets } }, select: { walletAddress: true, asset: true, available: true } }),
        ]);
    ...
    app.get('/api/admin/users/:wallet', async (request, reply) => {
        const admin = await (0, adminAuth_1.requireAdmin)(request, ['support_admin', 'finance_admin']);
        if (!admin)
            return reply.code(403).send({ error: 'Forbidden' });
        ...
        const [user, profile, balances, deposits, withdrawals, trades, reports] = await Promise.all([
            prisma_1.prisma.user.findFirst({ where: { primaryWalletAddress: wallet } }),
            prisma_1.prisma.profile.findFirst({ where: { walletAddress: wallet } }),
            prisma_1.prisma.userBalance.findMany({ where: { walletAddress: wallet } }),
            prisma_1.prisma.deposit.findMany({ where: { userWallet: wallet }, orderBy: { createdAt: 'desc' }, take: 50 }),
            prisma_1.prisma.withdrawalRequest.findMany({ where: { userWallet: wallet }, orderBy: { createdAt: 'desc' }, take: 50 }),
            prisma_1.prisma.trade.findMany({ where: { walletAddress: wallet }, orderBy: { timestamp: 'desc' }, take: 50 }),
            prisma_1.prisma.report.findMany({ where: { reporterWallet: wallet }, orderBy: { createdAt: 'desc' }, take: 50 }),
        ]);
        if (!user && !profile && balances.length === 0)
    ...
    app.get('/api/admin/tokens', async (request, reply) => {
        const admin = await (0, adminAuth_1.requireAdmin)(request, ['token_admin', 'support_admin', 'viewer']);
        if (!admin)
            return reply.code(403).send({ error: 'Forbidden' });
        const query = request.query;
        const { limit, offset } = page(request.query);
    ...
    app.get('/api/admin/finance/withdrawals', async (request, reply) => {
        const admin = await (0, adminAuth_1.requireAdmin)(request, ['finance_admin', 'support_admin', 'viewer']);
        if (!admin)
            return reply.code(403).send({ error: 'Forbidden' });
        const query = request.query;
        const { limit, offset } = page(request.query);
        const withdrawals = await prisma_1.prisma.withdrawalRequest.findMany({ where: query.status ? { status: query.status } : undefined, orderBy: { createdAt: 'desc' }, take: limit, skip: offset });
        return reply.send({ withdrawals: withdrawals.map(serializeBigInt) });
    });
    app.get('/api/admin/audit-logs', async (request, reply) => {
        const admin = await (0, adminAuth_1.requireAdmin)(request, ['super_admin', 'support_admin', 'viewer']);
        if (!admin)
            return reply.code(403).send({ error: 'Forbidden' });
        const { limit, offset } = page(request.query);
