...
async function runHotWalletWorker() {
    ...
    setInterval(async () => {
        ...
        // ── GRADUATIONS ──────────────────────────────────────────────────────────
        try {
            if (!bsc_1.GRADUATION_DEPLOYER_ADDRESS || bsc_1.GRADUATION_DEPLOYER_ADDRESS === '0x0000000000000000000000000000000000000000') {
                return;
            }
            ...
            try {
                const deployer = new ethers_1.ethers.Contract(bsc_1.GRADUATION_DEPLOYER_ADDRESS, DEPLOYER_ABI, wallet);
                const usdt = new ethers_1.ethers.Contract(bsc_1.PAYMENT_ASSET, ERC20_ABI, wallet);
                ...
                const trades = await prisma_1.prisma.trade.aggregate({
                    where: { tokenMint: graduatingToken.mint, type: 'buy' },
                    _sum: { paymentAmount: true },
                });
                const totalUsdtIn = BigInt(trades._sum.paymentAmount?.toString() ?? '0');
                const liquidityUsdt = totalUsdtIn > 0n ? totalUsdtIn : 15000000000n; // fallback $15k
                ...
                const transferTx = await usdt.transfer(bsc_1.GRADUATION_DEPLOYER_ADDRESS, liquidityUsdt);
                console.log(`[HotWallet] USDT transfer tx: ${transferTx.hash}`);
                await transferTx.wait(1);
                // Step 2: Call deployAndGraduate
                const tx = await deployer.deployAndGraduate(graduatingToken.name, graduatingToken.symbol, BigInt(graduatingToken.supplyCap.toString()), liquidityUsdt, { gasLimit: 5_000_000 });
                ...
                const deployerInterface = new ethers_1.ethers.Interface(DEPLOYER_ABI);
                let deployedTokenAddress = graduatingToken.tokenAddress || graduatingToken.mint;
                let dexPoolAddress = graduatingToken.tokenAddress || graduatingToken.mint;
