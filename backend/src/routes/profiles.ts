...
function getExponentialIntegral(supply: number, pMin: number, pMax: number, graduationThreshold: number): number {
  if (supply <= 0 || graduationThreshold <= 0 || pMin <= 0) return 0;
...
function getGrossSellValue(tokenAmount: number, currentSupply: number, pMin: number, pMax: number, graduationThreshold: number): number {
...
export async function profileRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { wallet: string } }>(
    ...
    async (req, reply) => {
      ...
      const lotsByToken = new Map<string, Array<{ amount: number; cost: number }>>();
      ...
      for (const trade of trades) {
        ...
        if (timestamp > previous) latestHolderTrade.set(trade.tokenMint, timestamp);
      }
      ...
      const holdings = netHoldings
        .map((h) => {
          ...
          const curveValue = hasExternalMarketMove
            ? getGrossSellValue(h.tokenAmount, supply, pMin, pMax, gt)
            : 0;
          ...
          const tradeMarkValue = hasExternalMarketMove && latestExternal && latestExternal.price > 0
            ? h.tokenAmount * latestExternal.price
            : 0;
          ...
          return {
            ...
            costBasis: Math.round(h.costBasis * 1e6) / 1e6,
